...
![img](https://sheungxin.github.io/notpic/849589-20180402133438219-1946132192.png)
...
```java
/**
 * 为一个没有重复元素的数组构建MaxTree，要求时间复杂度O(N) 空间复杂度O(N)
 */
public class MaxTree {
    ...
    static class Node {
        ...
        public String toString() {
            return "Node{" +
                    "value=" + value +
                    ", left=" + (left != null ? String.valueOf(left.value) : "") +
                    ", right=" + (right != null ? String.valueOf(right.value) : "") +
                    '}';
        }
    }
    ...
    public static Node maxTree(int[] arr) {
        ...
        // 左边首个较大的节点
        Map<Node, Node> lMap = new HashMap<>(8);
        // 右边首个较大的节点
        Map<Node, Node> rMap = new HashMap<>(16);
        // 遍历为每个数寻找左边首个较大的节点
        for (int i = 0; i < arr.length; i++) {
            Node curNode = nodes[i];
            // 顺序放入栈，遇到比栈顶大的值，依次弹出（此时栈中数据刚好有序，可以构建lMap）
            while (!stack.isEmpty() && stack.peek().value < curNode.value) {
                popStackAndSetMap(stack, lMap);
            }
            stack.push(curNode);
        }
        // 依次弹出剩余元素（此时栈中数据刚好有序，可以构建lMap）
        while (!stack.isEmpty()) {
            popStackAndSetMap(stack, lMap);
        }
        // 遍历为每个数寻找右边首个较大的节点（需要从右边开始）
        for (int i = arr.length - 1; i >= 0; i--) {
            Node curNode = nodes[i];
            // 顺序放入栈，遇到比栈顶大的值，依次弹出（此时栈中数据刚好有序，可以构建rMap）
            while (!stack.isEmpty() && stack.peek().value < curNode.value) {
                popStackAndSetMap(stack, rMap);
            }
            stack.push(curNode);
        }
        // 依次弹出剩余元素（此时栈中数据刚好有序，可以构建rMap）
        while (!stack.isEmpty()) {
            popStackAndSetMap(stack, rMap);
        }
        // 头节点
        Node head = null;
        // 根据lMap、rMap构建MaxTree
        for (int i = 0; i < arr.length; i++) {
            Node curNode = nodes[i];
            Node left = lMap.get(curNode);
            Node right = rMap.get(curNode);
            // 左右节点均为空，没有比它更大的节点，肯定是头节点
    ...
    /**
     * 弹出栈构建相邻两个元素的关系并放入map
     *
     * @param stack 栈
     * @param map   map
     */
    public static void popStackAndSetMap(Stack<Node> stack, Map<Node, Node> map) {
        // 弹出一个元素
        Node v = stack.pop();
        // 如果栈不为空，与下一个元素映射
        if (stack.isEmpty()) {
...
public class MaxRecSize {
    ...
    public static int maxRecFromBottom(int[] height) {
        ...
        for (int i = 0; i < height.length; i++) {
            /*
            把下标放入栈中，遇到栈顶下标对应高度大于等于当前高度的情况，依次弹出栈顶元素
            1、为什么遇到栈顶下标对应高度大于等于当前高度时弹出栈顶元素
            此时弹出栈顶元素，其对应高度构建的最大矩形右边界一定是i（因为i对应的高度较低），左边界一定是弹出后新的栈顶元素。
            左右边界直接的距离，乘于弹出下标对应高度，即为弹出下标对应高度的最大矩形面积
            2、在遇到栈顶下标对应高度等于当前高度时弹出栈顶元素，以i为右边界是否有误？
            的确，理论上右边界应该向右扩展，但由于相邻两个相同高度可以构建的最大矩形实际上是一样的，i还是要压入栈的，等i弹出时再计算即可
             */
            while (!stack.isEmpty() && height[stack.peek()] >= height[i]) {
                // 弹出栈顶元素，求下标j对应高度的最大矩形，左边界肯定为i（因为i对应的高度较低）
                int j = stack.pop();
                // 计算左边界位置，即新的栈顶值，栈为空记为-1
                int k = stack.isEmpty() ? -1 : stack.peek();
                // 左右边界的距离计算最大矩形面积，和当前最大面积比取最大值（不包含i、k位置，所以要减1）
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getMaxRecSize(data));
    }
}
```
...
```java
/**
 * 给定数组arr和整数num，返回满足max(arr[i,j])-min(arr[i,j])<=num的子数组数量
 */
public class MaxMinusMinInSubArray {
    ...
    public static int getNum(int[] arr, int num) {
        ...
        // 从i出发，找出满足条件的最大数组
        while (i < arr.length) {
            // 从j出发，逐步扩大数组范围，找出最大、最小值
            while (j < arr.length) {
                /*
                依次放入链表尾部，遇到尾部大于或等于当前元素的，先弹出尾部元素，保证后续弹出头部元素，新的头部还是最小
                注意：一定要>=，=不能缺失，当break后下一次进入还是从上一次的j开始，因此链表尾部元素和当前j有可能相同，不先弹出会出现重复放入的问题
                 */
                ...
                minList.addLast(j);
                /*
                依次放入链表尾部，遇到尾部小于或等于当前元素的，先弹出尾部元素，保证后续弹出头部元素，新的头部还是最大
                注意：一定要<=，=不能缺失，当break后下一次进入还是从上一次的j开始，因此链表尾部元素和当前j有可能相同，不先弹出会出现重复放入的问题
                 */
                ...
                maxList.addLast(j);
                // 若[i,j]之间最大值与最小值差已经大于num，停止数组范围扩大
                if (arr[maxList.getFirst()] - arr[minList.getFirst()] > num) {
                    break;
                }
                j++;
            }
            // 下次数组从i+1开始，头部等于i的需要弹出
            if (minList.peekFirst() != null && minList.peekFirst() == i) {
                minList.pollFirst();
            }
            // 下次数组从i+1开始，头部等于i的需要弹出
            if (maxList.peekFirst() != null && maxList.peekFirst() == i) {
                maxList.pollFirst();
            }
            System.out.println(i + "," + j);
            // 根据从i出发满足条件的最大数组，计算子数组数量（j是边界且不包含，同时单个i也算一个子数组，因此j-i即可）
            count += j - i;
            i++;
        }
        return count;
...
public class PublicLinked {
    ...
    /**
     * 打印两个有序链表的公共部分
     * 注意：此处是两个有序无环链表，相交节点开始之后部分一定相同
     *
     * @param head1 头结点1
     * @param head2 头结点2
     */
    public static void printPublicLink(Node head1, Node head2) {
        // 任何一个头结点为空直接返回
...
```java
/**
 * 在单链表中删除倒数第K个节点（双链表类似，注意双向指针即可）
 */
public class DelLastkLinked {
    static class Node {
        private int value;
        private Node next;
    ...
    /**
     * 删除单链表中倒数第K个元素
     * 解题思路：找到正数第K个节点firstK，从head、firstK同时出发，firstK达到尾结点，慢指针所在节点即为lastK
     *
     * @param node 头结点
     * @param k    倒数第K
     * @return 新的头节点
     */
    public static Node delLastK(Node node, int k) {
        if (node == null || k <= 0) {
            return node;
        }
        // 找到正数第K个节点
        ...
        for (int i = 1; i < k; i++) {
            if (firstK.next != null) {
                ...
                throw new RuntimeException("no last K node");
            }
        }
        // 待删除节点，从头结点开始移动
        Node delNode = node;
    ...
    /**
     * 删除单链表中倒数第K个元素
     * 解题思路：lastK=first(N-K+1)
     *
     * @param node 头结点
     * @param k    倒数第K
     * @return 新的头节点
     */
    public static Node delLastK2(Node node, int k) {
        if (node == null || k <= 0) {
            return node;
        }
        int n = k;
...
/**
 * 删除链表中间节点和a/b处的节点
 * 1、节点数量为1，没有中间节点，不用删除
 * 2、偶数节点，删除靠左的节点
 * 3、a/b定位节点默认向上取整
 */
public class DelNodeByPosition {
    static class Node {
        private int value;
        private Node next;
    ...
    public static Node delMiddleNode(Node head) {
        ...
        // 慢指针，每次移动一步
        Node pre = head;
        // 快指针，每次移动两步（默认先移动一次，这样到底尾部时，慢指针少移动一次，刚好是中间节点的前一个节点）
     ...
     * @return 新的头结点
     */
    public static Node delNode(Node head, int a, int b) {
        // 头结点为空、a/b>1、a<1直接返回
    ...
    public static void main(String[] args) {
        ...
        head1.next.next.next = new Node(6);
        head1.next.next.next.next = new Node(8);
        Node node = delMiddleNode(head1);
        while (node != null) {
            System.out.println(node.value);
            node = node.next;
        }
        System.out.println("-----------");
        Node node1 = delNode(head1, 3, 5);
        while (node1 != null) {
...
public class ReverseNode {
    ...
    public static void main(String[] args) {
        ...
        DoubleNode doubleNode4 = new DoubleNode(6);
        ...
        DoubleNode doubleHead = reverseDoubleNode(doubleNode1);
        while (doubleHead != null && doubleHead.last == last) {
            System.out.print(doubleHead.value + " ");
            last = doubleHead;
...
```java
/**
 * 给定一个单向链表的头节点，以及两个整数from和to，在单向链表上把第from个节点到第to个节点这一部分进行反转
 * 例如：1->2->3->4->5->null，from=2，to=4，调整后结果为：1->4->3->2->5->null
 */
public class ReversePartNode {
    ...
    public static Node reverse(Node head, int from, int to) {
        ...
        // 若从第一个节点开始反转，fPre为空，头结点为第一个节点
        Node pre = fPre == null ? head : fPre.next;
        // 从第二个节点开始
        cur = pre.next;
        // 第一个反转节点为尾节点，下一个节点为tPos，即反转截止的下一个节点
        pre.next = tPos;
        // 从第二个节点开始遍历直到反转范围截止节点tPos
        while (cur != tPos) {
            Node next = cur.next;
            cur.next = pre;
            pre = cur;
 ...
 * 成对反转单向链表
 */
public class ReversePairedNode {
    ...
    public static Node reverse(Node head) {
        ...
        while (cur != null && cur.next != null) {
            // 下一个节点
            Node next = cur.next;
            // 成对反转，当前节点应指向第三个节点（并非最终指向，此处仅保证节点之间不中断，下一次反转通过pre修复）
            cur.next = next.next;
            // 成对反转，next指向当前节点
            next.next = cur;
            // 本次反转后与前置节点连接
            ...
            // 重置前置节点
            pre = cur;
            // 重置当前节点（实际上是下一对的首个节点，cur.next指针已被修改过）
            cur = cur.next;
        }
        return newHead;
    }
...
public class ReverseNode {
    ...
    public static Node reverseByK(Node head, int k) {
        ...
        while (cur != null) {
            ...
            if (++ix == k) {
                ...
                // 本次逆序首个节点，即为前一次的下一个节点，首次为头结点
                start = pre == null ? head : pre.next;
                // 知道逆序范围及前后节点，逆序范围内节点
                reverse(pre, start, cur, cur.next);
                // 本次的开始节点逆序后即为下一次的前置节点
                pre = start;
                // 重新开始计数
                ix = 0;
            }
    ...
    private static void reverse(Node left, Node start, Node end, Node right) {
        // 左侧节点不为空，指向新的下一个节点，即逆序截止节点（逆序后即为开始节点，刚好和左节点连接）
        if (left != null) {
            left.next = end;
        }
        // 前置节点，首个前置节点即为右节点，因为start节点逆序后到尾部
        Node pre = right;
        Node next;
        // 从start开始遍历直到right截止
        while (start != right) {
    ...
    /**
     * 将单链表的每K个节点之间逆序（借助栈结构）
     *
     * @param head 头结点
     * @param k    连续逆序节点
     * @return 新的头结点
     */
    public static Node reverseByK2(Node head, int k) {
        // 头结点为空或者连续逆序节点小于2直接返回
        if (head == null || k < 2) {
            return head;
        }
        ...
        while (cur != null) {
            ...
            if (stack.size() == k) {
                ...
                // 首个弹出的元素作为前置节点
                popPre = popNode;
                // 首次满足栈数据量为k，第一个弹出节点即为新的头结点
                newHead = newHead == head ? popNode : newHead;
                // 左侧节点不为空，建立连接
                if (left != null) {
                    left.next = popNode;
                }
                ...
                while (!stack.isEmpty()) {
                    popNode = stack.pop();
                    // 注意：此处使用popPre指向popNode，因为栈后进先出
                    popPre.next = popNode;
                    // 重置popPre
                    popPre = popNode;
                }
                // 逆序后的节点指向右侧节点，结尾cur的下一个节点
                popPre.next = next;
                // 重置左侧节点
                left = popPre;
            }
     ...
     * @return 新的头结点
     */
    public static Node reverseByK3(Node head, int k) {
        // 头结点为空或者连续逆序节点小于2直接返回
        if (head == null || k < 2) {
            return head;
        }
        ...
        // 找不到尾部节点，即节点数量少于k个，不逆序直接返回头结点
        if (tail == null) {
            return head;
            // tail节点不为空，逆序head到tail节点
        } else {
            // head节点作为前置节点
            Node pre = head;
            // 从第二个节点开始
            cur = pre.next;
            // 首个前置节点逆序后为尾部节点，指向后续节点，后续节点利用递归处理
            pre.next = reverseByK3(tail.next, k);
            Node next;
            // 不能直接使用cur != tail.next，两个原因：cur有可能为空、tail参与逆序，tail.next会变化
            while (cur != tail) {
                next = cur.next;
                cur.next = pre;
                pre = cur;
 ...
 * 返回：最后生存下来的节点，且这个节点自己租车环形单向链表，其他节点都删除
 */
public class JosephusProblem {
    ...
    /**
     * 依次报数删除报数为m的节点，直到剩余一个节点
     * 时间复杂度O(N*M) N为节点数量，M为报数值
     *
     * @param head 头结点
     * @param m    报数值
     * @return 新的头结点
    ...
    /**
     * 依次报数删除报数为m的节点，直到剩余一个节点（与josephusKill1的差别在于先找到第一个前置节点）
     * 时间复杂度O(N*M) N为节点数量，M为报数值
     *
     * @param head 头结点
     * @param m    报数值
    ...
    public static Node josephusKill2(Node head, int m) {
        ...
        // 当前报数值
        int ix = 0;
        // 遍历直到前后两个节点一样，即只有一个节点为止，这里把head看做当前节点
        while (last != head) {
            // 报数为m，last作为前置节点，跨过当前节点head，执行head.next节点
            if (++ix % m == 0) {
                last.next = head.next;
            } else {
                // 重置前置节点
    ...
    /**
     * 根据节点数量N和报数值M推算出最终存活下来的节点，递推公式：f(N,M)=(f(N−1,M)+M-1)%N+1
     * 1、f(N,M)表示，N个人报数，每报到M时杀掉那个人，最终胜利者的编号
     * 2、f(N−1,M)表示，N-1个人报数，每报到M时杀掉那个人，最终胜利者的编号
     * 理解这个递推式的核心在于关注胜利者下标位置是怎么变的，每杀掉一个人需要重新计数，相当于把胜利者向前移动了M位，为了防止越界，模上当前人数。
     * 且由于编号是从1开始，因此需要先减一再取模得到胜利者下标，最后实际编号还需要加1
    ...
    public static int getLive(int i, int m) {
        ...
        // 只有一个节点即为胜利节点，反向推算2、3...N个节点时胜利者编号
    ...
    public static void main(String[] args) {
        ...
        Node node = josephusKill3(head1, 2);
        System.out.println(node);
    }
}
```
    ...
    public static int[] getNextArray(char[] ms) {
        ...
        while (pos < ms.length) {
            ...
            if (ms[pos - 1] == ms[cn]) {
                // 记录最大匹配长度，然后指针向右移动
                next[pos++] = ++cn;
                // 如果最大前缀和后缀的下一位不一致，向左找上一个最大前缀,例如：ABAABAB
            } else if (cn > 0) {
                // cn>0表示存在最大匹配前缀
                cn = next[cn];
            } else {
    ...
    public static int process(String[] strs, String str) {
        ...
        while (left < right) {
            ...
            if (strs[mid] != null && str.equals(strs[mid])) {
                ...
                if (strs[ix] == null || strs[ix].compareTo(str) < 0) {
                    left = mid + 1;
                    // 反之向左查找
                } else {
                    // 若首个不为空元素等于待查找元素，需要记录最右匹配位置
                    pos = strs[ix].equals(str) ? ix : pos;
                    // 调整右边界
                    right = ix - 1;
                }
            }
