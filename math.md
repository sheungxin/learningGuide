...
- 有理数：有限小数、无限循环小数
- 无理数：无限不循环小数，无法表示成分子和分母都是整数的分数，例如：$\pi$、$\sqrt{2}$、$log_23$
- 两个数进行四则运算
  - 一个无理数、一个有理数（乘法、除法时非0），则结果为无理数
  - 两个都是无理数，结果不确定 

- 等式中含有无理数，把原表达式拆分为有理项和无理项，等式为零时，系数要为零
...
- $S_n$为等差数列前n项和，则$S_n,S_{2n}-S_n,S_{3n}-S_{2n},...$仍是等差数列，公差为$n^2d$
...
- $S_n$为等比数列前n项和，则$S_n,S_{2n}-S_n,S_{3n}-S_{2n},...$仍是等比数列，公比为$q^n$
...
- 两点$P_1(x_1,y_1)$与点$P_2(x_2,y_2)$的中点坐标为$(\frac{x_1+x_2}{2},\frac{y_1+y_2}{2})$
- 两点距离公式$d=\sqrt{(x_1-x_2)^2+(y_1-y_2)^2}$
- 倾斜角是直线与$x$轴正方向所形成的夹角，该角的正切值为斜率，记为$k=\tan{a},a\ne\frac{\pi}{2}$
  - $a=0$，k=0
  - $0\lt a \lt\frac{\pi}{2}$，$k\gt 0$
  - $a=\frac{\pi}{2}$，k不存在
  - $\frac{\pi}{2}\lt a \lt \pi$，$k \lt 0$
  - $\tan{a}=\frac{\sin{a}}{\cos{}a}$，$\tan{2a}=\frac{2\tan{a}}{1-(\tan{a})^2}$
  - $k=\frac{y_1-y_2}{x_1-x_2},x_1\ne x_2$
- 直线方程
  - 斜截式：$y=kx+b$，其中$k$为斜率，$b$为截距
  - 点斜式：$y=y_0+k(x-x_0)$或$\frac{y-y_0}{x-x_0}=k$
  - 截距式：$\frac{x}{a}+\frac{y}{b}=1$
  - 两点式：$\frac{x-x_1}{x_2-x_1}=\frac{y-y_1}{y_2-y_1}$
  - 一般式：$ax+by+c=0$，斜率$k=-\frac{a}{b}$，与坐标轴围成三角形面积为$S=\frac{c^2}{2|ab|}$
- 直线位置关系：需要注意斜率为0或者不存在的情况
  - 平行：$k_1=k_2,c_1\ne c_2$
  - 相交：$k_1\ne k_2$
  - 垂直：$k_1k_2=-1$
...
- 点与直线的位置关系
  ...
  $y_0\begin{cases}\gt kx_0+b，点在直线上方 \\ =kx_0+b，点在直线上 \\ \lt kx_0+b，点在直线下方\end{cases}$

- 点到直线的距离$d=\frac{|ax_0+by_0+c|}{\sqrt{a^2+b^2}}$
- 两条平行直线的距离$d=\frac{|c_1-c_2|}{\sqrt{a^2+b^2}}$，注意：平行直线$k_1=k_2$，应用该公式前先统一两条直线$x、y$的系数
- 圆的方程
  - 标准式：$(x-x_0)^2+(y-y_0)^2=r^2$
  - 一般式：$x^2+y^2+ax+by+c=0$，配方后标准式为：$(x+\frac{a}{2})^2+(y+\frac{b}{2})^2=\frac{a^2+b^2-4c}{4}$，成立条件为：$a^2+b^2-4c\gt 0$

- 点与圆的关系，看点到圆心的距离

  $(x_p-x_0)^2+(y_p-y_0)^2=\begin{cases}\gt r^2，点P在圆外 \\ =r^2，点P在圆上 \\ \lt r^2，点P在圆内\end{cases}$

- 直线与圆的关系，使用圆心到直线的距离d进行判定
  - 直线与圆相离：$d\gt r$
  - 直线与圆相切：$d= r$
  - 直线与圆相交：$d\lt r$

- 圆与圆的关系，使用圆心之间的距离d进行判断
  - 外离：$d\gt r_1+r_2$，2条公共内切线，2条公共外切线
  - 外切：$d= r_1+r_2$，1条公共内切线，2条公共外切线
  - 相交：$r_1-r_2\lt d\lt r_1+r_2$，0条公共内切线，2条公共外切线
  - 内切：$d= r_1-r_2$，0条公共内切线，1条公共外切线
  - 内含：$d \lt r_1-r_2$，0条公共内切线，0条公共外切线

- 点P关于直线$l$：$ax+by+c=0$ 对称，对称点为Q
  - 对称点连成的直线与直线$l$垂直：$\frac{y_p-y_q}{x_p-x_q}*(-\frac{a}{b})=-1$
  - 对称点连成的直线中点在直线$l$上：$a*\frac{x_p+x_q}{2}+b*\frac{y_p+y_q}{2}+c=0$
  - 对称点Q万能公式：$(x_p-\frac{2D*a}{a^2+b^2},y_p-\frac{2D*b}{a^2+b^2})$，其中$D=ax_p+by_p+c$

- 直线关于直线的对称
  - 平行直线对称，直线之间距离相等（已知两条直线方程，利用直线间距离公式可推出第三条直线方程：$ax+by+(2c-c_1)=0$
  - 相交直线对称：先求出交点，再利用除交点外任意一点求出对称点，最后利用两点式求出对称直线方程

- 圆关于直线的对称：求出圆心关于直线的对称点，再由半径不变求出圆的方程
- 特殊的对称

| 对称方式       | 点$P(x_0,y_0)$  | 直线$l:ax+by+c=0$ | 规律                         |
| -------------- | --------------- | ----------------- | ---------------------------- |
| 关于$x$轴对称  | $P`(x_0,-y_0)$  | $l:ax-by+c=0$     | 将$y$换成$-y$                |
| 关于$y$轴对称  | $P`(-x_0,y_0)$  | $l:-ax+by+c=0$    | 将$x$换成$-x$                |
| 轴对称         | $P`(-x_0,-y_0)$ | $l:ax+by-c=0$     | 将$x$换成$-x$，将$y$换成$-y$ |
| 关于$y=x$对称  | $P`(y_0,x_0)$   | $l:ay+bx+c=0$     | 将$x$与$y$交换               |
| 关于$y=-x$对称 | $P`(-y_0,-x_0)$ | $l:ay+bx-c=0$     | 将$x$换成$-y$，将$y$换成$-x$ |

- 点$P(x,y)$关于点$M(a,b)$的的对称点Q的坐标为$Q(2a-x,2b-y)$
- 直线$l：Ax+By+C=0$关于点$P(a,b)$对称直线方程为：$A(2a-x)+B(2b-y)+C=0$
- 直线相关面积
  - 直线与坐标轴围成三角形面积为$S=\frac{c^2}{2|ab|}$
  - 两条直线与$x轴或y轴$围成的三角形面积，利用直线交点和与坐标轴的交点求面积（高使用点到直线的距离公式）
  - 三条直线围成的三角形面积，求出三个交点，利用两点间距离公式和点到直线的距离公式分别求出底和高

- 最值问题
  - 一个距离求最值：转化为圆的问题，先求出与圆心的距离，再加上或减去半径求最值
  - 两个距离之和求最值：采用对称思想，利用两点之间直线最短分析
  - 面积最值：例如三角形面积，列出函数表达式，结合抛物线或平均值定理分析最值
  - 线性规划的最值，例如：动点在多边形上运动的时候，找出构成多边形的交点直接代入目标函数（线性规划主要是一次函数）
  - 动点的最值：转化为某类图像求最值，例如：$ax+by、\frac{y-b}{x-a}、x^2+y^2$，可以分别视为斜率固定的平行直线、$(x,y)$到定点的斜率、圆到原点的距离的平方，上述表达式设K，解法如下
    - 联立$\Delta\ge 0$
    - 动点在圆上，相切求最值  
  - 三角形固定，C为中点，矩形面积最大
  - 矩形固定，三角形面积有最小值
...
- 长方体
  - 体积：$V=abc$
  - 全面积：$F=2(ab+ac+bc)$
  - 体对角线：$d=\sqrt{a^2+b^2+c^2}$

- 柱体：圆柱和棱柱侧面展开都是矩形
  - 侧面积：$S=底面周长*高$（展开矩形的面积），其中圆柱体$S=2\pi rh$
  - 体积：$V=底面积*高$，其中圆柱体$V=\pi r^2h$
  - 等边圆柱$h=2r$，只有等边圆柱才存在内切球（等边圆柱的体积与表面积之比=内切球的体积与表面积之比=3:2）
...
- 外接求和内切球
  ...
  | 几何体 | 内切球                                      | 外接球                       |
  | ------ | ------------------------------------------- | ---------------------------- |
  | 长方体 | 无，只有正方体才有                          | 体对角线$l=2R$               |
  | 正方体 | 棱长$a=2R$                                  | 体对角线$l=2R(2R=\sqrt{3}a)$ |
  | 圆柱   | 只有轴截面是正方形的圆柱才有，此时$2r=h=2R$ | $\sqrt{h^2+(2r)^2}=2R$       |
